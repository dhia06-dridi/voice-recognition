# real_time_voice_detector_mobilenet.py
import sys
import os
import numpy as np
import torch
...
print("=" * 60)
print("🚀 DÉMARRAGE DU DÉTECTEUR VOCAL - MobileNetV3")
print("=" * 60)
...
class MobileNetVoiceDetector:
    """Détecteur vocal optimisé pour MobileNetV3"""
    
    def __init__(self, onnx_path, labels_path, confidence_threshold=0.6):
        print(f"\n🔧 Initialisation du détecteur MobileNetV3...")
        ...
        # Configuration audio POUR MOBILENETV3
        self.sample_rate = 16000
        self.target_length = 16000  # 1 seconde
        self.n_fft = 1024
        self.hop_length = 256
        self.n_mels = 128
        self.f_min = 20
        ...
        try:
            ...
            print("✅ Transforms audio créés pour MobileNetV3")
        except Exception as e:
            raise Exception(f"Échec création transforms: {e}")
        
        # Setup ONNX pour MobileNetV3
        self._setup_onnx(onnx_path)
        
        # État et smoothing
        self.smoothing_factor = 0.3  # Plus agressif pour MobileNet
        self.previous_probs = np.zeros(self.num_classes)
        self.vad_threshold = 0.003   # Plus sensible
        self.vad_history = deque(maxlen=8)
        self.noise_floor = 0.0005
        self.last_detection_time = 0
        self.min_detection_interval = 0.8  # Plus long pour éviter les faux positifs
        
        # Cache pour performances
        self.last_spectrogram = None
        
        print("✅ Détecteur MobileNetV3 initialisé!\n")
    ...
    def _setup_onnx(self, onnx_path):
        try:
            ...
            print(f"📐 Shape d'entrée MobileNetV3: {self.input_shape}")
            
            # Vérifier la compatibilité
            if len(self.input_shape) != 4 or self.input_shape[1] != 3:
                print("⚠️  Attention: Shape d'entrée peut nécessiter ajustement")
    ...
    def has_voice_activity(self, audio_data):
        ...
        if len(self.vad_history) > 0:
            ...
            self.noise_floor = min(avg_energy * 0.8, self.noise_floor * 1.005)
        
        self.vad_history.append(energy)
        
        # Seuil dynamique basé sur l'historique
        dynamic_threshold = self.noise_floor + self.vad_threshold
        return energy > dynamic_threshold and energy > 0.001
    
    def preprocess_audio(self, audio_data):
        """Prétraitement optimisé pour MobileNetV3"""
        try:
            ...
            # Ajuster longueur exactement à 1 seconde
            current_length = waveform.shape[1]
            if current_length < self.target_length:
                # Padding réflexif pour meilleure qualité
            ...
            if mel_std < 1e-8:
                mel_std = 1.0
            mel_spec_db = (mel_spec_db - mel_mean) / mel_std
            
            # Ajuster à la taille attendue par MobileNetV3 (128x128 typiquement)
            target_height, target_width = 128, 128
            
            current_height, current_width = mel_spec_db.shape[1], mel_spec_db.shape[2]
            
            # Redimensionnement intelligent
            if current_width < target_width:
                # Répéter si trop petit
            ...
            if current_height < target_height:
                # Padding vertical si nécessaire
                pad_bottom = target_height - current_height
                mel_spec_db = F.pad(mel_spec_db, (0, 0, 0, pad_bottom), mode='constant', value=mel_spec_db.min())
            elif current_height > target_height:
                # Crop vertical si nécessaire
                mel_spec_db = mel_spec_db[:, :target_height, :]
            
            # 3 canaux pour MobileNetV3
            mel_spec_3ch = mel_spec_db.repeat(3, 1, 1)
            
            # Cache pour optimisation
            self.last_spectrogram = mel_spec_3ch
            ...
            # Fallback: spectrogramme vide
            return np.zeros((1, 3, 128, 128), dtype=np.float32)
    
    def apply_temporal_smoothing(self, current_probs):
        """Smoothing temporel amélioré"""
        if np.sum(self.previous_probs) == 0:
            self.previous_probs = current_probs
            return current_probs
        
        # Smoothing adaptatif basé sur la confiance
        confidence = np.max(current_probs)
        adaptive_smoothing = self.smoothing_factor * (1.0 - confidence * 0.5)
        
        smoothed = (adaptive_smoothing * self.previous_probs + 
                   (1 - adaptive_smoothing) * current_probs)
    ...
    def predict(self, audio_data):
        """Prédiction avec optimisation MobileNetV3"""
        try:
            # Vérifier activité vocale
            ...
            # Vérifier la forme
            if input_tensor.shape != (1, 3, 128, 128):
                print(f"⚠️  Shape inattendu: {input_tensor.shape}")
                return None, 0.0, self.previous_probs, []
            ...
            top_probs, top_indices = probs_tensor.topk(top_k, dim=1)
            ...
            # Gestion du temps entre détections
...
class RealTimeVoiceDetectorUI(QMainWindow):
    ...
    def __init__(self):
        ...
        print("\n🎨 Initialisation de l'interface MobileNetV3...")
        self.init_ui()
        self.load_model()
        print("✅ Interface MobileNetV3 initialisée\n")
        
    def init_ui(self):
        self.setWindowTitle("🎤 Détecteur Vocal MobileNetV3 - Design Moderne")
        ...
        title = QLabel("🎤 DÉTECTEUR VOCAL MOBILENETV3")
        ...
        self.model_label = QLabel("🧠 Model: MobileNetV3")
        self.model_label.setFont(QFont("Arial", 10))
        self.model_label.setStyleSheet("color: #3498db; font-weight: bold;")
        status_layout.addWidget(self.model_label, 0, 4)
    ...
    def load_model(self):
        """Charger le modèle ONNX MobileNetV3"""
        try:
            self.detector = MobileNetVoiceDetector(
                onnx_path='speech_recognition_model.onnx',
                labels_path='class_labels.json',
                confidence_threshold=0.6
            )
            
            self.log_message("✅ Modèle MobileNetV3 chargé avec succès!")
            ...
            self.log_message(f"❌ Erreur de chargement MobileNetV3: {e}")
    ...
    def start_monitoring(self):
        ...
        try:
            ...
            self.log_message("🎙️ Détection MobileNetV3 démarrée - Parlez maintenant!")
...
def main():
    ...
    try:
        print("\n🚀 Démarrage de l'application MobileNetV3...")
        ...
        print("\n" + "=" * 60)
        print("🎤 APPLICATION MOBILENETV3 PRÊTE!")
        print("=" * 60)
        print("💡 Cliquez sur 'DÉMARRER' et prononcez des mots de commande")
        print("🧠 Modèle: MobileNetV3 optimisé pour la voix")
